...
- Ported the old `Recipe$SpongeToDeadbloodRecipe` brewing surface through
...
- Added the evidence-backed infected InfSheep/Assimilated Sheep slice:
  - confirmed the old infected-sheep line uses visible entity id `sim_sheep`,
    ...
    spawn egg item `itemmobspawner_infsheep`. `EntityInfSheepHead` is tracked
    in its own head-entity slice below, while `EntityFerSheep` remains a
...
- Added the evidence-backed infected InfSheepHead / Walking Sheep Head slice:
  - confirmed the old head line uses visible entity id `sim_sheephead`,
    `EntityInfSheepHead`, renderer `RenderInfSheepHead`, model
    `ModelInfSheepHead`, texture `assets/srparasites/textures/entity/monster/sheeph.png`,
    and spawn egg item `itemmobspawner_infsheephead`,
  - upgraded `itemmobspawner_infsheephead` from a placeholder item into a real
    modern spawn egg using the established old assimilated colors `8611072`
    (`0x835000`) and `16711900` (`0xFF00DC`),
  - preserved parasite ID `22`, size `0.7 x 0.7`, eye height `0.6`, shadow
    radius `0.5`, legacy `killcount = -10`, attack speed marker `15`, reduced
    fall-damage multiplier `0.3`, and the old `canSpawnByIDData` relationship
    to the InfSheep natural-spawn gate. The inspected 1.10.6 defaults leave
    `INFSHEEP_HEADHEALTH` and `INFSHEEP_HEADDAMAGE` at `0.0` before config head
    multipliers; the modern runtime uses a small non-zero health/damage
    fallback so the registered entity is playable until the full config
    multiplier table is migrated,
  - preserved the visible old AI surface: swimming/float handling, look idle,
  ...
  - wired legacy head sounds `INFECTEDHEAD_GROWL`, `INFECTEDHEAD_HURT`, and
    ...
    existing `small.step` sound event,
  - wired a GeckoLib client renderer to the converted legacy
    `ModelInfSheepHead` geometry, Java-authored animation resource, and
    jar-backed `sheeph.png` texture. The converted model keeps the 64x46 texture
    size, 108 bones, and the two legacy pose-mutating animation methods
    `animation.inf_sheep_head.func_78087_a`,
    `animation.inf_sheep_head.setRotationAnglesCosmical`,
  - documented the old `EntityInfSheepHead` special `disloGiveBodies` tick path
    and crude-host recombination with `EntityInhooM`: the 1.10.6 head could
    become `EntityInfSheep` through dislodgement world phase data, and could
    attack a crude `EntityInhooM`, spawn/replace into `EntityInfSheep`,
    particle-status the crude host, and discard the crude host. The current
    repo has no `SRPSaveData` world phase backend and no crude `EntityInhooM`
    implementation, so those two body-recombination paths are left as explicit
    future backend slices instead of being faked.
...
- InfSheep/Assimilated Sheep's full `EntityPInfected` backend,
  ...
  variant surface,
- InfSheepHead/Walking Sheep Head's exact legacy head health/damage config
  multiplier table, full `EntityPInfected` backend, exact `EntityAISkill`
  scheduler internals, exact `disloGiveBodies` + `SRPSaveData` body-upgrade
  path, and special crude-host recombination with `EntityInhooM` remain
  explicit future slices. The current repo has no `SRPSaveData` world phase
  backend and no crude `EntityInhooM` implementation, so the modern slice
  preserves the registered `sim_sheephead` entity, spawn egg, visible AI
